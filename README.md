...
When creating a room, the host can check **Homerun scoring**. With it on: if you beat a given opponent in all 3 hand categories on a single board with no ties (a clean sweep), that board's points against them double — 6 points becomes 12 (or −12 if they sweep you). A board where you split the categories scores normally either way. The results screen shows a banner and a badge on any player who hits one.

Empirically (see `experiments/solve/` below), this measurably changes optimal strategy: without the modifier, you should dump your weakest card in the 1-card slot since it's low-leverage; with it, the 1-card hand becomes the bottleneck for completing a 3-way sweep, so it's often worth reinforcing it with a stronger card instead.
