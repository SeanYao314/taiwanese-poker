...
Stuck, or just want to check your work? **Show Ideal Split** exhaustively scores all 105 ways to split your 7 cards against your trial set and shows you the best one, with a "Load Ideal Split" button to drop it straight into your slots. This is a real (if brute-force) search, not a heuristic. It always caps itself at the first 1,000 boards of your trial set regardless of where the slider above is set — past that point the winning partition essentially stops changing, so spending minutes re-confirming the same answer at 10,000 boards isn't worth it. When the cap kicks in, the panel says so explicitly (and shows how many of your boards it actually used) so the ideal split's EV and your own arrangement's EV aren't silently being compared across different-sized samples. Cost scales with the board count up to that cap — a few seconds at 150 boards, up to ~25s at 1,000 — so it runs in small batches with a live progress readout on the button instead of freezing the tab for the whole computation, and it's cached per hand so asking again is instant.
