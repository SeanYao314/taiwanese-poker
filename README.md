...
Note on board count and opponent diversity: the pre-solved opponent pool only has ~1120 hands total, of which roughly a third qualify (don't share a card with yours) for any given practice hand — comfortably enough for the default 150, but pushing toward the 1000 max means opponents start repeating across trials (always with fresh boards each time, never a duplicate card within a single trial). More boards still means less noise in the EV estimate; it just doesn't buy more opponent variety past a few hundred.

Stuck, or just want to check your work? **Show Ideal Split** exhaustively scores all 105 ways to split your 7 cards against those same trials and shows you the best one, with a "Load Ideal Split" button to drop it straight into your slots. This is a real (if brute-force) search, not a heuristic. Cost scales with the board count — a few seconds at 150 boards, up to ~25s at the 1000 max — so it runs in small batches with a live progress readout on the button instead of freezing the tab for the whole computation, and it's cached per hand so asking again is instant.
