...
Stuck, or just want to check your work? **Show Ideal Split** exhaustively scores all 105 ways to split your 7 cards against your trial set and shows you the best one, with a "Load Ideal Split" button to drop it straight into your slots. This is a real (if brute-force) search, not a heuristic. It has its own separate **Ideal split search boards** slider — also logarithmic, 100–10,000 — independent from the EV panel's slider above it, since the search is much more expensive per board (105 partitions vs. 1) and the winning partition tends to stop changing well before 1,000 boards; defaults to 1,000 but you're free to push it higher for a slower, more precise search, or lower for a quicker (noisier) one. It's clamped down to however many boards this hand actually has (set by the EV slider at deal time), and the panel says so explicitly whenever that clamp kicks in, so the ideal split's EV and your own arrangement's EV aren't silently being compared across different-sized samples. Cost scales with however many boards the search slider asks for — a few seconds at 150, up to roughly half a minute at 1,000, longer if you push it further — so it runs in small batches with a live progress readout on the button instead of freezing the tab for the whole computation. Changing the search slider invalidates any cached answer for the current hand (asking again recomputes at the new count); leaving it alone means asking again is instant.
